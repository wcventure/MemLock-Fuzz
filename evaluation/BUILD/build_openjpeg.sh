...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	echo "start ..."
	cd ${ROOT_DIR}/evaluation/BUILD/openjpeg
    git clone https://github.com/uclouvain/openjpeg SRC
    cd SRC
    git checkout 51f097e6d5754ddae93e716276fe8176b44ec548
    cd ..
    rm -rf $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock
    rm -rf $(dirname ${BIN_PATH})/openjpeg/SRC_AFL
    mv $(dirname ${BIN_PATH})/openjpeg/SRC $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock
    cp -rf $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock $(dirname ${BIN_PATH})/openjpeg/SRC_AFL
	...
	cd $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock
	if [ -d "$(dirname ${BIN_PATH})/openjpeg/SRC_MemLock/build"  ]; then
		rm -rf $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock/build
	fi
    ...
    mkdir $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock/build
    cd $(dirname ${BIN_PATH})/openjpeg/SRC_MemLock/build
	...
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
	cd $(dirname ${BIN_PATH})/openjpeg/SRC_AFL
	if [ -d "$(dirname ${BIN_PATH})/openjpeg/SRC_AFL/build"  ]; then
        rm -rf $(dirname ${BIN_PATH})/openjpeg/SRC_AFL/build
    fi
    export CC=${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-clang-fast
    export CXX=${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-clang-fast++
    export CFLAGS="-g -O0 -fsanitize=address"
    export CXXFLAGS="-g -O0 -fsanitize=address"
    mkdir $(dirname ${BIN_PATH})/openjpeg/SRC_AFL/build
    cd $(dirname ${BIN_PATH})/openjpeg/SRC_AFL/build
    cmake -G "Unix Makefiles" -DBUILD_SHARED_LIBS=off ..
	make
