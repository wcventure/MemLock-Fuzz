...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-heap-fuzz -i ${ROOT_DIR}/evaluation/BUILD/readelf/SEED/ -o out_MemLock$i -m none -d -t 5000 --  ${ROOT_DIR}/evaluation/BUILD/readelf/SRC_MemLock/build/bin/readelf -a @@
fi
