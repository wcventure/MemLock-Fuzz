...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/exiv2/SRC_MemLock/build/bin" ]; then
	...
	for ((i=1; i<100; i++))
	do
		if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/exiv2/out_MemLock$i" ]; then
			break
		fi
	done
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-heap-fuzz -i ${ROOT_DIR}/evaluation/BUILD/exiv2/SEED/ -o out_MemLock$i -m none -d -t 5000 --  ${ROOT_DIR}/evaluation/BUILD/exiv2/SRC_MemLock/build/bin/exiv2 -pX @@
fi
