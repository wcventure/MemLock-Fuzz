...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/yaml-cpp/SRC_MemLock/build/util" ]; then
		${ROOT_DIR}/evaluation/BUILD/build_yaml-cpp.sh
	fi
	echo "start ..."
	cd ${ROOT_DIR}/evaluation/FUZZ
	if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/yaml-cpp" ]; then
		mkdir yaml-cpp
	fi
	cd yaml-cpp
	i=0
	for ((i=1; i<100; i++))
	do
		if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/yaml-cpp/out_MemLock$i" ]; then
			break
		fi
	done
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=1:abort_on_error=1:symbolize=0:detect_leaks=0
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-stack-fuzz -i ${ROOT_DIR}/evaluation/BUILD/yaml-cpp/SEED/ -o out_MemLock$i -m none -d --  ${ROOT_DIR}/evaluation/BUILD/yaml-cpp/SRC_MemLock/build/util/parse @@
fi
