...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/openjpeg/SRC_AFL/build/bin" ]; then
	...
	for ((i=1; i<100; i++))
	do
		if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/openjpeg/out_AFL$i" ]; then
			break
		fi
	done
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
	${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/evaluation/BUILD/openjpeg/SEED/ -o out_AFL$i -m none -d -t 5000 --  ${ROOT_DIR}/evaluation/BUILD/openjpeg/SRC_AFL/build/bin/opj_decompress -i @@ -o ./tmp.png
fi
