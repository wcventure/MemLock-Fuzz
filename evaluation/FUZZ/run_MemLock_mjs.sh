...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/mjs/SRC_MemLock/build" ]; then
		${ROOT_DIR}/evaluation/BUILD/build_mjs.sh
	fi
	echo "start ..."
	cd ${ROOT_DIR}/evaluation/FUZZ
	if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/mjs" ]; then
		mkdir mjs
	fi
	cd mjs
	i=0
	for ((i=1; i<100; i++))
	do
		if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/mjs/out_MemLock$i" ]; then
			break
		fi
	done
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=1:abort_on_error=1:symbolize=0:detect_leaks=0
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-stack-fuzz -i ${ROOT_DIR}/evaluation/BUILD/mjs/SEED/ -o out_MemLock$i -m none -d --  ${ROOT_DIR}/evaluation/BUILD/mjs/SRC_MemLock/build/mjs @@
fi
