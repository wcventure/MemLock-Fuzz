...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	${ROOT_DIR}/tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/evaluation/BUILD/readelf/SEED/ -o out_AFL$i -m none -d -t 5000 --  ${ROOT_DIR}/evaluation/BUILD/readelf/SRC_AFL/build/bin/readelf -a @@
fi
