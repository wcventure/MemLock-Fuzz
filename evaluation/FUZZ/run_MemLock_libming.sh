...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
	...
	if ! [ -d "${ROOT_DIR}/evaluation/BUILD/libming/SRC_MemLock/build/bin" ]; then
		${ROOT_DIR}/evaluation/BUILD/build_libming.sh
	fi
	echo "start ..."
	cd ${ROOT_DIR}/evaluation/FUZZ
	if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/libming" ]; then
		mkdir libming
	fi
	cd libming
	i=0
	for ((i=1; i<100; i++))
	do
		if ! [ -d "${ROOT_DIR}/evaluation/FUZZ/libming/out_MemLock$i" ]; then
			break
		fi
	done
	export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=1
	${ROOT_DIR}/tool/MemLock/build/bin/memlock-heap-fuzz -i ${ROOT_DIR}/evaluation/BUILD/libming/SEED/ -o out_MemLock$i -m none -d -t 5000 --  ${ROOT_DIR}/evaluation/BUILD/libming/SRC_MemLock/build/bin/listswf @@
fi
